...
const GOOGLE_SHEET_CSV_URL =
  "https://docs.google.com/spreadsheets/d/e/2PACX-1vTyh1AJApeD-UUcEwJvsEj7IgozJzjGzUXv8OY3wOPGD71_HbhsfuHUJcPb3uFC9-rnpCLE2j2YE7DK/pub?output=csv";
...
// --- ROBUST CSV LINE PARSER (ΔΕΝ σπάει σε κενά, σέβεται quotes) ---
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  // --- ΣΩΣΤΗ ΤΟΠΙΚΗ ΗΜΕΡΟΜΗΝΙΑ (όχι UTC) ---
  ...
  if (dateDisplay) dateDisplay.textContent = todayObj.toLocaleDateString("el-GR", options);
  ...
  async function fetchGoogleSheet() {
    try {
      ...
      // Καθαρό split με υποστήριξη Windows/Mac newlines
      ...
      rows.forEach((row) => {
        ...
        let idsRaw = cols[2];
        // αφαίρεση περιμετρικών quotes αν υπάρχουν
        idsRaw = idsRaw.replace(/^"|"$/g, "");
  ...
  function initApp() {
    ...
    function renderContent() {
      ...
      if (cityTitle) cityTitle.textContent = `Εφημερεύει: ${currentArea}`;

      // Εύρεση εφημερίας με ευέλικτη αναζήτηση (χωρίς τόνους/κενά)
      const scheduleEntry = globalSchedule.find(
        (s) => s.date === todayStr && normalize(s.area) === normalize(currentArea)
      );
        ...
        uniqueSubAreas.forEach((sub) => {
          ...
          if (hasPharmacy) {
            ...
            activePharmasInSub.forEach((pharma, index) => {
              const mapLink = pharma.map
                ? pharma.map
                : `https://www.google.com/maps/search/?api=1&query=${encodeURIComponent(
                    pharma.name + " " + pharma.address + " " + pharma.area
                  )}`;

              if (index > 0)
                detailsHTML += '<hr style="margin: 15px 0; border: 0; border-top: 1px solid #eee;">';
