// --- ΡΥΘΜΙΣΕΙΣ ---
const SHOW_ALL_MODE = true; // Βάλτο false όταν βάλεις πρόγραμμα

// Mapping: Ποιο SubArea θεωρείται το "Κέντρο" για κάθε περιοχή;
const cityCenters = {
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (SHOW_ALL_MODE) {
        ...
        dateDisplay.innerHTML += " <br><span style='font-size:0.8em; color:#e67e22;'>(Test Mode)</span>";
    } else {
    ...
    function renderContent() {
        ...
        // 2. Διαχωρισμός: Βρες τα φαρμακεία της Περιοχής
        const areaPharmacies = pharmacies.filter(p => p.area === currentArea);
        
        // Ποιο είναι το όνομα του κέντρου; (π.χ. "Έδεσσα (Κέντρο)")
        const centerName = cityCenters[currentArea];

        // 3. --- ΕΜΦΑΝΙΣΗ ΚΕΝΤΡΙΚΟΥ ΦΑΡΜΑΚΕΙΟΥ (Featured) ---
        // Βρες ποια ενεργά φαρμακεία ανήκουν στο κέντρο
        const activeCenterPharmacies = activePharmacies.filter(p => p.area === currentArea && p.subArea === centerName);
        ...
        if (activeCenterPharmacies.length > 0) {
            ...
            cityContainer.innerHTML = `
                <div class="featured-card" style="background:#f9f9f9; border-top-color:#ccc;">
                    <p style="color:#777;">Δεν βρέθηκε εφημερία στο κέντρο (${centerName}) για σήμερα.</p>
                </div>`;
        }


        // 4. --- ΕΜΦΑΝΙΣΗ ΛΙΣΤΑΣ ΧΩΡΙΩΝ (Τα υπόλοιπα) ---
        // Βρες τα χωριά που ΔΕΝ είναι το κέντρο
        const uniqueSubAreas = [...new Set(areaPharmacies.map(p => p.subArea))]
            .filter(sub => sub !== centerName) // Αφαίρεσε το κέντρο από τη λίστα
            .sort();

        if (uniqueSubAreas.length === 0) {
            gridContainer.innerHTML = '<p style="text-align:center; color:#999;">Δεν υπάρχουν χωριά σε αυτή την περιοχή.</p>';
            return;
        }
        ...
        uniqueSubAreas.forEach(sub => {
                ...
                detailsHTML = `
                    <div class="location-details">
                        <div class="details-content">
                            ...
                            <p style="margin:0 0 10px;"><i class="fas fa-map-marker-alt"></i> ${activePharmaInSub.address}</p>
            ...
            // Click Event μόνο αν υπάρχει φαρμακείο
            if(activePharmaInSub) {
